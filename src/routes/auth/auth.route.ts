...
} from "../../shared-types/schema/benutzer.schema";
import {
  loginController,
  registerController
...
authRouter.put('/change-password', authenticateToken, validateRequest({body: changePasswordSchema}), changePassword)
authRouter.post('/request-new-password', validateRequest({body: requestNewPasswordSchema}), requestNewPasswordController)
authRouter.post('/login/:token', validateRequest({
  params: loginParams,
  body: loginSchema
}), changePasswordAndLoginController)

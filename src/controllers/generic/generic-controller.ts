...
import {mayReadFilterQuery, mayWriteFilterQuery} from "./filter-queries";

export class GenericController {
  static search<T>(Model: ReturnModelType<any>, regExProps: string[] = ['name'], permissionFilter = false) {
    return (req: Request, res: Response) => {
      const filterQuery = permissionFilter ? mayReadFilterQuery(req) : {}
  ...
  static delete(Model: ReturnModelType<any>, permissionFilter = false) {
    return (req: Request, res: Response) => {
      const permissionFilterQuery = permissionFilter ? mayWriteFilterQuery(req) : {}
  ...
  static put<T>(Model: ReturnModelType<any>, handleAuthorization = false) {
    return (req: Request, res: Response) => {
      const authorizeFilterQuery = handleAuthorization ? mayWriteFilterQuery(req) : {}
